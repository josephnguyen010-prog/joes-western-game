...
var ROUTE=[[44,0],[-44,0],[-58,-14],[-58,-38],[10,-46],[46,-34],[54,-10]];
...
function updateCar(dt){
  ...
  if(td<6.5) car.wp=(car.wp+1)%ROUTE.length;
  var off=angWrap(Math.atan2(tx,tz)-car.yaw);
  car.steer=damp(car.steer,clamp(off*1.7,-1,1),6,dt);
  car.hurry=damp(car.hurry,1,0.5,dt);

  var want=(Math.abs(off)>0.7?4.2:9.6)*car.hurry;
  var s=car.speed;
