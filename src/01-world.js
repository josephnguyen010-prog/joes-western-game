...
var AU={
  ...
  init:function(){
    ...
    this.noise=b;
    var w=c.createBufferSource(); w.buffer=b; w.loop=true;
    var wf=c.createBiquadFilter(); wf.type='bandpass'; wf.frequency.value=420; wf.Q.value=0.7;
    var wg=c.createGain(); wg.gain.value=0.035;
    var lfo=c.createOscillator(); lfo.frequency.value=0.09;
    var lg=c.createGain(); lg.gain.value=0.022;
...
var PLANKS=[
  plank('#8a6a45','#4a3220'), plank('#9c8358','#5a442a'), plank('#6f5940','#3a2b1c'),
  plank('#a8926b','#63492e'), plank('#7d6248','#412e1e')
];
...
function building(x,z,face,w,d,h,name,kind,lowFront){
  var S=Structure(x,z,face), g=S.g;
  var shell=pick(PLANKS), open=!!kind;
  var wallMat=shell;
  ...
  if(open){
    ...
    for(var sg=-1;sg<=1;sg+=2){                                       // side walls, cut for a window
      ...
      S.part(0.05,y1-y0-0.10,ow-0.10,MAT.pane,sx,(y0+y1)/2,swz,false);
    ...
    S.part(bside,by1-by0,WALLT,wallMat, (bow+bside)/2,(by0+by1)/2,d/2-WALLT/2);
    S.part(bow-0.10,by1-by0-0.10,0.05,MAT.pane,0,(by0+by1)/2,d/2-0.13,false);
    ...
    for(var sgn=-1;sgn<=1;sgn+=2){
      ...
      if(ww>0.7){
        ...
        S.part(ww-0.05,wh-0.06,0.035,MAT.pane,cx,wy,-d/2+0.13,false);  // glazing
    ...
    interiors.push({x0:x-iw,x1:x+iw,z0:z-id,z1:z+id});
  }else{
  ...
  var fh=h+(lowFront?0.95:rr(1.4,2.4));
  S.part(w+0.34,fh-h,0.36,shell,0,(h+fh)/2,-d/2-0.14,false);         // false front: parapet only
  ...
  S.part(w+0.34,0.20,0.42,MAT.wood1,0,h+0.02,-d/2-0.16,false);       // cornice under the parapet
...
function dressSaloon(S,g,w,d,h,v){
  ...
  if(v===0){
    ...
    picture('landscape',1.15,0.86,-w/2+WALLT+0.10,2.30,d*0.10,Math.PI/2);
    picture('portrait',0.66,0.84,w/2-WALLT-0.10,2.30,-d*0.10,-Math.PI/2);
    picture('still',0.70,0.86,w/2-WALLT-0.10,2.30,d*0.20,-Math.PI/2);
...
var TOWN_S=[
  ['OCCIDENTAL SALOON','saloon0',13.5],['DRY GOODS','store',11.5],['HOTEL DRURY','hotel',12],
  ['BARBER','barber',10],['LIVERY & FEED','livery',12],['BULLHEAD SALOON','saloon2',12],['BLACKSMITH','forge',11]
];
var TOWN_N=[
  ['BANK OF CIMARRON','bank',12.5],['J. PRINE UNDERTAKER','undertaker',11],['ASSAY OFFICE','office',10.5],
  ['WESTERN UNION','telegraph',10],['THE PALACE','saloon1',12],['MILLINERY','millinery',10],['DOC MERRIWEATHER','doctor',10.5]
];
var PERCH=null;
function saloonRoof(bx,bz,w,d,h){
  ...
  var sz=bz+d/2+1.35, x0=bx-3.1, i, sy;
  for(i=0;i<14;i++){                                  // stair up the back
    sy=(i+1)*(rY/14);
    box(0.48,0.16,1.30,MAT.wood1,x0+i*0.46,sy-0.08,sz);
    platforms.push({x0:x0+i*0.46-0.24,x1:x0+i*0.46+0.24,z0:sz-0.65,z1:sz+0.65,y:sy});
    if(i%3===0){
      box(0.10,1.05,0.10,MAT.wood1,x0+i*0.46,sy+0.52,sz+0.60);
      box(0.10,1.05,0.10,MAT.wood1,x0+i*0.46,sy+0.52,sz-0.60);
    }
  }
  var lx=x0+13.5*0.46;
  box(1.6,0.16,2.9,MAT.wood1,lx,rY-0.08,sz-0.75);     // landing onto the roof
  platforms.push({x0:lx-0.8,x1:lx+0.8,z0:sz-2.2,z1:sz+0.7,y:rY});
  box(0.10,1.05,0.10,MAT.wood1,lx+0.7,rY+0.52,sz+0.55);
  box(1.6,0.10,0.10,MAT.wood1,lx,rY+1.0,sz+0.60);
  var sign=box(1.5,0.5,0.08,MAT.wood1,lx,rY+1.0,sz-2.0);
  sign.visible=false;
}
(function buildTown(){
  var xs=-48,i,e;
  for(i=0;i<TOWN_S.length;i++){
    e=TOWN_S[i];
    var bd=rr(9.5,11), bh=rr(4.7,5.5), bx=xs+e[2]/2, bz=14+bd/2;
    if(i===0) bh=5.6;
    building(bx,bz,0,e[2],bd,bh,e[0],e[1],i===0);
  ...
  for(i=0;i<TOWN_N.length;i++){
    e=TOWN_N[i];
    building(xs+e[2]/2,-14-rr(9.5,11)/2,Math.PI,e[2],rr(9.5,11),rr(4.7,5.5),e[0],e[1]);
    xs+=e[2]+rr(1.3,2.6);
  }
})();
...
(function church(){
  ...
  S.part(2.2,H-2.9,WALLT,inMat,0,(H+2.9)/2,-D/2+WALLT/2,false);
  ...
  interiors.push({x0:CX-D/2+0.4,x1:CX+D/2-0.4,z0:CZ-W/2+0.4,z1:CZ+W/2-0.4});
...
(function dressTheSet(){
  ...
  for(i=0;i<9;i++){                                   // a few landmark outcrops, well clear of town
    var la=rr(0,TAU), lr=rr(58,80);
    rock(Math.cos(la)*lr,Math.sin(la)*lr,rr(1.5,2.6),MAT.cliff);
  }
  for(i=0;i<4;i++) skull(rr(-70,70),rr(-70,70));
  // boulders piled at the foot of the cliffs so the edge of the world reads as rock
  for(i=0;i<52;i++){
    var a2=rr(0,TAU), r2=rr(WORLD_R+1,WORLD_R+12);
    rock(Math.cos(a2)*r2,Math.sin(a2)*r2,rr(1.6,4.4),MAT.cliff,true);
  }
})();
